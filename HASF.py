...
class CWF_block(nn.Module):  #通道权重融合
    # 初始化, in_channel代表特征图的输入通道数, b和gama代表公式中的两个系数
    def __init__(self, in_channel, b=1, gama=2):
        # 继承父类初始化
        super(CWF_block, self).__init__()
...
class SCA(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_SA = (k_shared_projected.permute(0, 1, 3, 2) @ (q.transpose(1, 2)))
...
class Block(nn.Module):
    def __init__(self, num_attention_heads, hidden_size): # , input_size
        ...
        self.ffn = CWF_block(3)
        self.sca = SCA(num_attention_heads, hidden_size)
